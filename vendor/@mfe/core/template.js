 ...
 */
export function loadTemplate(html, doc) {
    const d = doc ?? (typeof document !== 'undefined' ? document : undefined);
    if (!d) {
        throw new Error('loadTemplate: no Document available. Pass one explicitly when there is no global document (e.g. in SSR).');
    }
